...
public class PettyCashRequestManagement extends BorderPane {
    
    private String currentUser;

    private TableView<PettyCashRecordModel> pettyCashRequestTable;
    private ObservableList<PettyCashRecordModel> pettyCashRequestData = FXCollections.observableArrayList();
    ...
    public PettyCashRequestManagement(String username) {
    ...
    private VBox createHeader() {
        ...
        Text departmentName = new Text("PETTY CASH REQUEST MANAGEMENT");
    ...
    private HBox createFooter() {
        ...
        Text footerText = new Text("© 2025 Afran General Hospital - Smart HRMS | Petty Cash Management System v3.0 | Biometric Signature Integration");
    ...
    private void updateSummary() {
        if (pettyCashRequestData.isEmpty()) {
            summaryLabel.setText("No petty cash requests found");
            return;
        }

        long totalRequests = pettyCashRequestData.size();
        long pendingApproval = pettyCashRequestData.stream()
            .filter(r -> "Pending".equals(r.getApprovalStatus()))
            .count();
        long approved = pettyCashRequestData.stream()
            .filter(r -> "Approved".equals(r.getApprovalStatus()))
            .count();
        long confirmed = pettyCashRequestData.stream()
            .filter(r -> "Confirmed".equals(r.getConfirmationStatus()))
            .count();
        long dispensed = pettyCashRequestData.stream()
            .filter(r -> "Yes".equals(r.getDispensedStatus()))
            .count();
        double totalAmount = pettyCashRequestData.stream()
    ...
    private VBox createButtonPanel() {
        ...
        dispenseBtn.setOnAction(e -> {
            PettyCashRecordModel selected = pettyCashRequestTable.getSelectionModel().getSelectedItem();
            if (selected != null) {
    ...
    private void loadPettyCashRequestsFromDatabase() {
        new Thread(() -> {
            try {
                List<PettyCashRecordModel> requests = databaseConnector.getAllPettyCashRequests();
                Platform.runLater(() -> {
                    pettyCashRequestData.clear();
                    pettyCashRequestData.addAll(requests);
                    System.out.println("Loaded " + requests.size() + " petty cash requests from database");
                    updateSummary();
                    
                    if (!pettyCashRequestData.isEmpty()) {
                        pettyCashRequestTable.getSelectionModel().selectFirst();
                        updateButtonStates(pettyCashRequestTable.getSelectionModel().getSelectedItem());
                    }
                });
            } catch (Exception e) {
                Platform.runLater(() -> 
...
private void showPettyCashRequestForm() {
    Dialog<PettyCashRecordModel> dialog = new Dialog<>();
    ...
    List<String> mainCategories = databaseConnector.getPettyCashMainCategories();
    ...
    mainCategoryComboBox.valueProperty().addListener((obs, oldVal, newVal) -> {
        ...
        if (newVal != null) {
            List<String> subCategories = databaseConnector.getPettyCashSubCategories(newVal);
    ...
    grid.addRow(3, reasonLabel, reasonField);
    grid.addRow(4, payeeLabel, payeeComboBox);
    grid.addRow(5, amountLabel, amountField);
    grid.addRow(6, requestDateLabel, requestDateField);
    ...
    requisitionUnitField.textProperty().addListener((obs, oldVal, newVal) -> validateForm.run());
    reasonField.textProperty().addListener((obs, oldVal, newVal) -> validateForm.run());
    payeeComboBox.valueProperty().addListener((obs, oldVal, newVal) -> validateForm.run());
    amountField.textProperty().addListener((obs, oldVal, newVal) -> validateForm.run());

    // ---- FIXED MAXIMUM + AVAILABLE BALANCE VALIDATION ----
    okButton.addEventFilter(ActionEvent.ACTION, event -> {
        ...
        double maximumPettyCashRequest = databaseConnector.getMaximumRequest("petty cash");
        double availablePettyCashAmount = databaseConnector.getAvailablePettyCashAmount();

        // 1️⃣ Check maximum allowed
        if (amount > maximumPettyCashRequest) {
            showAlert("Error",
                "Requested amount exceeds the allowed petty cash limit.\n" +
                "Maximum Allowed: " + maximumPettyCashRequest + " ETB",
                Alert.AlertType.ERROR);
            event.consume();
            return;
        }

        // 2️⃣ Correct logic: Check if amount is MORE than available
        if (amount > availablePettyCashAmount) {
            showAlert("Error",
                "Requested amount exceeds available petty cash balance.\n" +
                "Available: " + availablePettyCashAmount + " ETB",
                Alert.AlertType.ERROR);
            event.consume();
        }
    });
    ...
    dialog.setResultConverter(dialogButton -> {
        if (dialogButton == ButtonType.OK) {
           ...
           return new PettyCashRecordModel(
    requestId,
    ...
    Optional<PettyCashRecordModel> result = dialog.showAndWait();

    // SAVE ONLY IF VALID
    result.ifPresent(newRecord -> {
        boolean saved = databaseConnector.savePettyCashRequest(newRecord);
        if (saved) {
            pettyCashRequestData.add(0, newRecord);
            pettyCashRequestTable.getSelectionModel().select(newRecord);
            updateSummary();
            showAlert("Success",
                "New petty cash request created successfully!\nRequest ID: " + newRecord.getRequestId(),
...
private String generateRequestId() {
    ...
    String monthPart = LocalDate.now().format(monthFormatter);
    
    int maxNumber = pettyCashRequestData.stream()
        .filter(record -> record.getRequestId() != null && record.getRequestId().startsWith(prefix + monthPart))
        .map(record -> record.getRequestId())
        .map(id -> id.substring(prefix.length() + monthPart.length()))
        .filter(part -> part.matches("\\d+"))
        .mapToInt(Integer::parseInt)
        .max()
        .orElse(0);
    
    return prefix + monthPart + String.format("%04d", maxNumber + 1);
}
// Examples: PC2023120001, PC2023120002 (resets monthly)
...
private void voidSelected() {
    ...
    if (result.isPresent() && !result.get().isEmpty()) {
        ...
        boolean updated = databaseConnector.voidPettyCashRequest(
    ...
    private void deleteSelected() {
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            ...
            if (deleted) {
                pettyCashRequestData.remove(selected);
                updateSummary();
    ...
    private void showRequestApprovalWithBiometricDialog(PettyCashRecordModel record) {
    ...
    private void showDispenseApprovalWithBiometricDialog(PettyCashRecordModel record) {
        Dialog<ButtonType> dialog = new Dialog<>();
    ...
    private void showConfirmationWithBiometricDialog(PettyCashRecordModel record) {
        Dialog<ButtonType> dialog = new Dialog<>();
    ...
    private void showDispenseForm(PettyCashRecordModel record) {
        Dialog<ButtonType> dialog = new Dialog<>();
    ...
    private void saveDispensedRecordWithDualSignatures(DispensedPettyCashModel record, 
        ...
        new Thread(() -> {
            try {
                boolean success = databaseConnector.saveDispensedPettyCashRecordWithDualSignatures(
                ...
                Platform.runLater(() -> {
                    if (success) {
                        ...
                        // Find and update the corresponding PettyCashRecordModel
                        PettyCashRecordModel updatedRecord = findPettyCashRecordById(record.getRequestId());
    ...
    // Helper method to find PettyCashRecordModel by request ID
    private PettyCashRecordModel findPettyCashRecordById(String requestId) {
        return pettyCashRequestData.stream()
            .filter(record -> record.getRequestId().equals(requestId))
            .findFirst()
            .orElse(null);
    }

    private void saveRequestApprovalWithBiometric(PettyCashRecordModel record, EmployeeSignatureModel approver, 
                                         byte[] fingerprintTemplate, String notes) {
        new Thread(() -> {
            try {
                boolean success = databaseConnector.savePettyCashRequestApprovalWithBiometric(
    ...
    private void saveDispenseApprovalWithBiometric(PettyCashRecordModel record, EmployeeSignatureModel approver, 
                                         byte[] fingerprintTemplate, String notes) {
        new Thread(() -> {
            try {
                boolean success = databaseConnector.savePettyCashDispenseApprovalWithBiometric(
                    record, approver, fingerprintTemplate, notes
                );
    ...
    private void saveConfirmationWithBiometric(PettyCashRecordModel record, EmployeeSignatureModel confirmer, 
                                             byte[] fingerprintTemplate, String notes) {
        new Thread(() -> {
            try {
                boolean success = databaseConnector.savePettyCashConfirmationWithBiometric(
    ...
    private void updateButtonStates(PettyCashRecordModel selected) {
        if (selected == null) {
    ...
    private void updateButtonTooltips(PettyCashRecordModel selected) {
...
private void viewRecordDetails(PettyCashRecordModel record) {
    if (record == null) {
...
//        logoView.setPreserveRatio(true);
        ...
        statusGrid.addRow(5,
            createBoldLabel("Void Reason:"), createStatusLabel(getSafeString(databaseConnector.getPettyCashRequestVoidReason(record.getRequestId()))));
...
private void loadSignaturesForDetails(PettyCashRecordModel record, HBox signaturesContainer) {
...
//    private String getSignatureTitle(String owner, PettyCashRecordModel record) {
...
//    }
    ...
    private boolean hasAllSignatures(PettyCashRecordModel record) {
    try {
    ...
    private void printDateRangeTabular(LocalDate fromDate, LocalDate toDate) {
        List<PettyCashRecordModel> filteredData = pettyCashRequestData.stream()
    ...
    private VBox createDetailedPrintNode(PettyCashRecordModel record, String title) {
    ...
    private VBox createTabularPrintNode(List<PettyCashRecordModel> data, LocalDate fromDate, LocalDate toDate) {
        VBox printNode = new VBox(15);
        ...
        // Table
        TableView<PettyCashRecordModel> printTable = new TableView<>();
        printTable.setItems(FXCollections.observableArrayList(data));
        
        TableColumn<PettyCashRecordModel, String> reqIdCol = new TableColumn<>("Request ID");
        ...
        TableColumn<PettyCashRecordModel, Double> amountCol = new TableColumn<>("Amount");
    ...
    private VBox createDateRangeDetailedPrintNode(List<PettyCashRecordModel> data, LocalDate fromDate, LocalDate toDate) {
        VBox printNode = new VBox(15);
        ...
        for (int i = 0; i < Math.min(data.size(), 5); i++) { // Limit to 5 records for printing
            PettyCashRecordModel record = data.get(i);
    ...
    private VBox createRecordPrintBox(PettyCashRecordModel record, int index) {
    ...
    private void exportToExcelWithData(List<PettyCashRecordModel> data, File file, LocalDate fromDate, LocalDate toDate) {
        new Thread(() -> {
            try (Workbook workbook = new XSSFWorkbook()) {
                ...
                // Data rows
                int rowNum = 4;
                for (PettyCashRecordModel record : data) {
                    Row row = sheet.createRow(rowNum++);
                ...
                summaryCell.setCellValue("SUMMARY: Total Requests: " + data.size() + 
                    " | Total Amount: ETB " + data.stream().mapToDouble(PettyCashRecordModel::getAmountRequested).sum());
    ...
    private void exportToExcelWithImages(List<PettyCashRecordModel> data, File file, LocalDate fromDate, LocalDate toDate) {
    ...
    private void exportToWordWithData(List<PettyCashRecordModel> data, File file, LocalDate fromDate, LocalDate toDate) {
        new Thread(() -> {
            try (XWPFDocument document = new XWPFDocument()) {
                // Title
                XWPFParagraph titleParagraph = document.createParagraph();
                ...
                summaryRun.setText("Summary: Total Requests: " + data.size() + 
                    ...
                    data.stream().mapToDouble(PettyCashRecordModel::getAmountRequested).sum()));
    ...
    private void exportToWordWithImages(List<PettyCashRecordModel> data, File file, LocalDate fromDate, LocalDate toDate) {
    ...
    private VBox createRequestDetailsSection(PettyCashRecordModel record) {
...
filterBox.getChildren().addAll(
        ...
        pettyCashRequestTable = new TableView<>();
        pettyCashRequestTable.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY);
        pettyCashRequestTable.setStyle("-fx-background-color: #ecf0f1; -fx-border-color: #bdc3c7; -fx-border-radius: 5;");

        // Proper row factory for double-click functionality
pettyCashRequestTable.setRowFactory(tv -> {
    TableRow<PettyCashRecordModel> row = new TableRow<PettyCashRecordModel>() {
        @Override
        protected void updateItem(PettyCashRecordModel item, boolean empty) {
    ...
    row.setOnMouseClicked(event -> {
        if (event.getClickCount() == 2 && !row.isEmpty()) {
            PettyCashRecordModel selectedRecord = row.getItem();
    ...
    return row;
        ...
        TableColumn<PettyCashRecordModel, String> requestIdCol = createStyledColumn("Request ID", "requestId", "#2c3e50");
        TableColumn<PettyCashRecordModel, String> requisitionUnitCol = createStyledColumn("Requisition Unit", "requisitionUnit", "#34495e");
        TableColumn<PettyCashRecordModel, String> reasonCol = createStyledColumn("Reason", "reason", "#16a085");
        TableColumn<PettyCashRecordModel, String> payeeCol = createStyledColumn("Payee", "payee", "#27ae60");
        TableColumn<PettyCashRecordModel, Double> amountRequestedCol = createStyledColumn("Amount Requested", "amountRequested", "#2980b9");
        TableColumn<PettyCashRecordModel, LocalDate> requestDateCol = createStyledColumn("Request Date", "requestDate", "#8e44ad");
        TableColumn<PettyCashRecordModel, String> approvalStatusCol = createStyledColumn("Approval Status", "approvalStatus", "#c0392b");
        TableColumn<PettyCashRecordModel, String> approvedByCol = createStyledColumn("Approved By", "approvedBy", "#7f8c8d");
        TableColumn<PettyCashRecordModel, String> confirmationStatusCol = createStyledColumn("Confirmation Status", "confirmationStatus", "#f39c12");
        TableColumn<PettyCashRecordModel, String> confirmedByCol = createStyledColumn("Confirmed By", "confirmedBy", "#d35400");
        TableColumn<PettyCashRecordModel, String> dispensedStatusCol = createStyledColumn("Dispensed Status", "dispensedStatus", "#16a085");
        TableColumn<PettyCashRecordModel, String> dispensedByCol = createStyledColumn("Dispensed By", "dispensedBy", "#27ae60");
        TableColumn<PettyCashRecordModel, String> dispenseApprovalStatusCol = createStyledColumn("Dispense Approval Status", "dispenseApprovalStatus", "#d35400");
        TableColumn<PettyCashRecordModel, String> dispenseApprovedByCol = createStyledColumn("Dispense Approved By", "dispenseApprovedBy", "#8e44ad");
        TableColumn<PettyCashRecordModel, String> voidStatusCol = createStyledColumn("Void Status", "voidStatus", "#e74c3c");
        TableColumn<PettyCashRecordModel, String> voidedByCol = createStyledColumn("Voided By", "voidedBy", "#c0392b");

        pettyCashRequestTable.getColumns().addAll(
        ...
        pettyCashRequestTable.setItems(pettyCashRequestData);
        ...
        pettyCashRequestTable.setPlaceholder(placeholder);

        // Proper selection listener
        pettyCashRequestTable.getSelectionModel().selectedItemProperty().addListener((obs, oldSel, newSel) -> {
            if (newSel != null) {
        ...
        tableSection.getChildren().addAll(filterBox, pettyCashRequestTable);
        return tableSection;
    }
    
    
    private void exportSelectedRecordToPDF(PettyCashRecordModel record) {
    if (record == null) {
   ...
   private <T> TableColumn<PettyCashRecordModel, T> createStyledColumn(String title, String property, String color) {
    TableColumn<PettyCashRecordModel, T> column = new TableColumn<>(title);
    column.setCellValueFactory(new PropertyValueFactory<>(property));
    column.setStyle("-fx-background-color: " + color + "; -fx-text-fill: white; -fx-font-weight: bold;");
    
    column.setCellFactory(tc -> new TableCell<PettyCashRecordModel, T>() {
        @Override
    ...
    pettyCashRequestTable.setItems(
    pettyCashRequestData.filtered(record -> {
    ...
    private void refreshData() {
        ...
        showAlert("Refreshed", "Data refreshed successfully!\n" + 
                  "Loaded " + pettyCashRequestData.size() + " petty cash requests and " + 
    ...
    private void performAdvancedSearch(String requestId, String payee, String requisitionUnit, 
        ...
        pettyCashRequestTable.setItems(pettyCashRequestData.filtered(record -> {
        ...
        showAlert("Search Complete", 
            "Found " + pettyCashRequestTable.getItems().size() + " matching records.",
            Alert.AlertType.INFORMATION);
    }

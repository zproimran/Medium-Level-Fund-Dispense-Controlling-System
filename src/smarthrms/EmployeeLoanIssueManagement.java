...
public class EmployeeLoanIssueManagement extends BorderPane {
    private String currentUser;

    private TableView<EmployeeLoanIssueModel> issueTable;
    private ObservableList<EmployeeLoanIssueModel> issueData = FXCollections.observableArrayList();

    public EmployeeLoanIssueManagement(String username) {
    ...
    private VBox createTableSection() {
        ...
        statusFilter.getItems().addAll("All", "Pending", "Issued", "Returned");
        ...
        TableColumn<EmployeeLoanIssueModel, String> idCol = new TableColumn<>("Issue ID");
        idCol.setCellValueFactory(new PropertyValueFactory<>("issueId"));

        TableColumn<EmployeeLoanIssueModel, String> empCodeCol = new TableColumn<>("Employee Code");
        empCodeCol.setCellValueFactory(new PropertyValueFactory<>("employeeCode"));
        
        TableColumn<EmployeeLoanIssueModel, String> empNameCol = new TableColumn<>("Employee Name");
        empNameCol.setCellValueFactory(new PropertyValueFactory<>("employeeName"));

        TableColumn<EmployeeLoanIssueModel, String> loanTypeCol = new TableColumn<>("Loan Type");
        loanTypeCol.setCellValueFactory(new PropertyValueFactory<>("loanType"));

        TableColumn<EmployeeLoanIssueModel, Double> amountCol = new TableColumn<>("Amount");
        amountCol.setCellValueFactory(new PropertyValueFactory<>("amount"));

        TableColumn<EmployeeLoanIssueModel, LocalDate> issueDateCol = new TableColumn<>("Issue Date");
        issueDateCol.setCellValueFactory(new PropertyValueFactory<>("issueDate"));

        TableColumn<EmployeeLoanIssueModel, String> statusCol = new TableColumn<>("Status");
        statusCol.setCellValueFactory(new PropertyValueFactory<>("status"));

        issueTable.getColumns().addAll(idCol, empCodeCol,empNameCol ,loanTypeCol, amountCol, issueDateCol, statusCol);
        issueTable.setItems(issueData);
        issueTable.setPlaceholder(new Label("No loan issues found"));

        tableSection.getChildren().addAll(filterBox, issueTable);
        return tableSection;
    }

    private void showIssueForm(EmployeeLoanIssueModel record) {
        Dialog<ButtonType> dialog = new Dialog<>();
        dialog.setTitle(record == null ? "Add Loan Issue" : "Edit Loan Issue");
        dialog.setHeaderText(record == null ? "Add New Loan Issue" : "Edit Loan Issue");
        ...
        DatePicker issueDate = new DatePicker(record == null ? LocalDate.now() : record.getIssueDate());
        ...
        grid.addRow(4, new Label("Issue Date:"), issueDate);
        ...
        dialog.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK) {
                if (record == null) {
                    EmployeeLoanIssueModel newRecord = new EmployeeLoanIssueModel(
                            "ISS" + (issueData.size() + 1),
                    ...
                    record.setIssueDate(issueDate.getValue());
                }
                issueTable.refresh();
            }
        });
    ...
    private void editSelected() {
        EmployeeLoanIssueModel selected = issueTable.getSelectionModel().getSelectedItem();
        if (selected != null) showIssueForm(selected);
    ...
    private void deleteSelected() {
        ...
        if (selected != null) {
            Alert confirm = new Alert(Alert.AlertType.CONFIRMATION,
                    "Are you sure you want to delete this loan issue?", ButtonType.YES, ButtonType.NO);
            confirm.showAndWait().ifPresent(response -> {
                if (response == ButtonType.YES) issueData.remove(selected);
            });
        }
    }
    ...
    private void loadSampleData() {
        issueData.add(new EmployeeLoanIssueModel("ISS001","EMP001", "John Doe", "Personal", 5000.0, LocalDate.of(2025,9,16), "Issued"));
        issueData.add(new EmployeeLoanIssueModel("ISS002","EMP002", "Jane Smith", "Education", 10000.0, LocalDate.of(2025,9,14), "Issued"));
    }
}

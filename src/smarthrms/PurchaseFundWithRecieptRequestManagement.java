...
public class PurchaseFundWithRecieptRequestManagement extends BorderPane {
    private String currentUser;

    private TableView<PurchaseFundRecordModel> purchaseFundRequestTable;
    private ObservableList<PurchaseFundRecordModel> purchaseFundRequestData = FXCollections.observableArrayList();
    ...
    private Button uploadRecieptBtn;
    ...
    public PurchaseFundWithRecieptRequestManagement(String username) {
    ...
    private VBox createHeader() {
        ...
        header.setStyle("-fx-background-color: linear-gradient(to right, #4c5e50, #3738db); -fx-padding: 20; -fx-alignment: center;");
        
        Text departmentName = new Text("RECIEPT BASED PURCHASE FUND MANAGEMENT");
    ...
    private HBox createFooter() {
        ...
        Text footerText = new Text("© 2025 Afran General Hospital - Smart HRMS | Reciept Based Purchase Fund Management System v3.0 | Biometric Signature Integration");
    ...
    private void updateSummary() {
        if (purchaseFundRequestData.isEmpty()) {
            summaryLabel.setText("No Reciept based purchase fund requests found");
            return;
        }

        long totalRequests = purchaseFundRequestData.size();
        long pendingApproval = purchaseFundRequestData.stream()
            .filter(r -> "Pending".equals(r.getApprovalStatus()))
            .count(); 
        long approved = purchaseFundRequestData.stream()
            .filter(r -> "Approved".equals(r.getApprovalStatus()))
            .count();
        long confirmed = purchaseFundRequestData.stream()
            .filter(r -> "Confirmed".equals(r.getConfirmationStatus()))
            .count();
        long dispensed = purchaseFundRequestData.stream()
            .filter(r -> "Yes".equals(r.getDispensedStatus()))
            .count();
        double totalAmount = purchaseFundRequestData.stream()
    ...
    private VBox createButtonPanel() {
        ...
        uploadRecieptBtn = createStyledButton("Upload Reciept", "#2980b9");
        uploadRecieptBtn.setOnAction(e -> uploadRecieptForSelected());
        ...
        dispenseBtn.setOnAction(e -> {
            PurchaseFundRecordModel selected = purchaseFundRequestTable.getSelectionModel().getSelectedItem();
            if (selected != null) {
        ...
        buttonPanel1.getChildren().addAll(
            requestBtn, editBtn,uploadRecieptBtn, voidBtn, deleteBtn, viewDetailsBtn, new Separator(),
        ...
        buttonPanel2.getChildren().addAll(
            ...
            buttonPanel1.getChildren().addAll(
            uploadRecieptBtn, voidBtn,viewDetailsBtn, new Separator(),
            dispenseBtn
        );
        buttonPanel2.getChildren().addAll(
            connectDeviceBtn, disconnectDeviceBtn, new Separator(),
    ...
    private void loadPurchaseFundRequestsFromDatabase() {
        new Thread(() -> {
            try {
                List<PurchaseFundRecordModel> requests = databaseConnector.getAllRecieptBasedPurchaseFundRequests();
                Platform.runLater(() -> {
                    purchaseFundRequestData.clear();
                    purchaseFundRequestData.addAll(requests);
                    System.out.println("Loaded " + requests.size() + "purchase Fund requests from database");
                    updateSummary();
                    
                    if (!purchaseFundRequestData.isEmpty()) {
                        purchaseFundRequestTable.getSelectionModel().selectFirst();
                        updateButtonStates(purchaseFundRequestTable.getSelectionModel().getSelectedItem());
                    }
                });
            } catch (Exception e) {
                Platform.runLater(() -> 
...
private void showPurchaseFundRequestForm() {
    ...
    Dialog<PurchaseFundRecordModel> dialog = new Dialog<>();
    dialog.setTitle("New Receipt-Based Purchase Fund Request");
    dialog.setHeaderText("Create New Receipt-Based Purchase Fund Request");
    ...
    List<String> mainCategories = databaseConnector.getMainCategories();
    ...
    mainCategoryComboBox.valueProperty().addListener((obs, oldVal, newVal) -> {
        ...
        if (newVal != null) {
            List<String> subCategories = databaseConnector.getSubCategories(newVal);
    ...
    reasonField.setPromptText("Enter reason/description for purchase fund request...");

    ComboBox<String> payeeComboBox = new ComboBox<>();
    payeeComboBox.setPromptText("Select payee...");
    ...
    grid.addRow(1, new Label("Main Category*:"), mainCategoryComboBox);
    grid.addRow(2, new Label("Sub Category*:"), subCategoryComboBox);
    grid.addRow(3, new Label("Reason/Description*:"), reasonField);
    grid.addRow(4, new Label("Payee*:"), payeeComboBox);
    ...
    Runnable validateForm = () -> {
        boolean isValid =
                ...
                mainCategoryComboBox.getValue() != null &&
                subCategoryComboBox.getValue() != null &&
    ...
    requisitionUnitField.textProperty().addListener((o, old, now) -> validateForm.run());
    mainCategoryComboBox.valueProperty().addListener((o, old, now) -> validateForm.run());
    subCategoryComboBox.valueProperty().addListener((o, old, now) -> validateForm.run());
    reasonField.textProperty().addListener((o, old, now) -> validateForm.run());
    payeeComboBox.valueProperty().addListener((o, old, now) -> validateForm.run());
    amountField.textProperty().addListener((o, old, now) -> validateForm.run());

    // --- Maximum amount validation BEFORE dialog close ---
    okButton.addEventFilter(ActionEvent.ACTION, event -> {
        String amountText = amountField.getText();

        double maxAllowed = databaseConnector.getMaximumRequest("reciept based purchase fund");
        double availableBalance = databaseConnector.getAvailableRecieptBasedPurchaseFundAmount();
        ...
        if (amount > maxAllowed) {
            showAlert("Amount Exceeded",
                    "Requested amount exceeds the maximum allowed limit.\n" +
                            "Maximum Allowed: " + maxAllowed + " ETB",
                    Alert.AlertType.ERROR);
            event.consume();
            return;
        }
        ...
        if (amount > availableBalance) {
            showAlert("Insufficient Balance",
                    "Requested amount exceeds the available balance.\n" +
                            "Available Amount: " + availableBalance + " ETB",
                    Alert.AlertType.ERROR);
            event.consume();
        }
    });
    ...
    dialog.setResultConverter(dialogButton -> {
        if (dialogButton == ButtonType.OK) {
            ...
            return new PurchaseFundRecordModel(
                    requestId,
                    requisitionUnitField.getText(),
                    mainCategoryComboBox.getValue(),
    ...
    Optional<PurchaseFundRecordModel> result = dialog.showAndWait();

    result.ifPresent(newRecord -> {
        boolean saved = databaseConnector.saveRecieptBasedPurchaseFundRequest(newRecord);

        if (saved) {
            purchaseFundRequestData.add(0, newRecord);
            purchaseFundRequestTable.getSelectionModel().select(newRecord);
            updateSummary();

            showAlert("Success",
                    "New purchase fund request created!\nRequest ID: " + newRecord.getRequestId(),
                    Alert.AlertType.INFORMATION);
        } else {
            showAlert("Error", "Failed to save request.", Alert.AlertType.ERROR);
        }
    });
}
...
private String generateRequestId() {
    String prefix = "RCBPF"; // Receipt prefix
    DateTimeFormatter dateFormatter = DateTimeFormatter.ofPattern("yyyyMMdd");
    String datePart = LocalDate.now().format(dateFormatter);
    
    int maxNumber = purchaseFundRequestData.stream()
...
// Examples: RCBPF20231215001, RCBPF20231215002
    ...
    private void editSelected() {
        ...
        dialog.setTitle("Edit Reciept Based Purchsae Fund Request");
        ...
        dialog.showAndWait().ifPresent(confirmed -> {
            if (confirmed) {
                ...
                boolean updated = databaseConnector.updateRecieptBasedPurchaseFundRequest(selected);
                if (updated) {
                    purchaseFundRequestTable.refresh();
                    updateSummary();
   ...
   private void uploadRecieptForSelected() {
    PurchaseFundRecordModel selected = purchaseFundRequestTable.getSelectionModel().getSelectedItem();
    if (selected == null) {
        showAlert("No Selection", "Please select a request to upload receipt.", Alert.AlertType.WARNING);
        return;
    }

    if ("Yes".equalsIgnoreCase(selected.getVoidStatus())) {
        showAlert("Cannot Upload", "Voided requests cannot have receipts uploaded.", Alert.AlertType.WARNING);
        return;
    }

    if (!"Approved".equalsIgnoreCase(selected.getApprovalStatus())) {
        showAlert("Not Approved", "Only approved requests can have receipts uploaded.", Alert.AlertType.WARNING);
        return;
    }

    if ("Uploaded".equalsIgnoreCase(selected.getRecieptUploadStatus())) {
        showAlert("Already Uploaded", "Receipts have already been uploaded for this request.", Alert.AlertType.INFORMATION);
        return;
    }

    // Show fingerprint verification first
    showFingerprintVerificationDialog(employee -> {
        if (employee != null) {
            // Proceed with file selection after successful verification
            Platform.runLater(() -> selectAndUploadReceipts(selected, employee.getEmployeeName()));
        }
    });
}

private void selectAndUploadReceipts(PurchaseFundRecordModel record, String uploadedBy) {
    FileChooser fileChooser = new FileChooser();
    fileChooser.setTitle("Select Receipt Files");
    ...
    if (selectedFiles != null && !selectedFiles.isEmpty()) {
        if (selectedFiles.size() > 10) {
            showAlert("Too Many Files", "Maximum 10 files allowed per request.", Alert.AlertType.WARNING);
            return;
        }
        ...
        if (totalSize > 50 * 1024 * 1024) {
            showAlert("File Size Exceeded", "Total file size exceeds 50MB limit.", Alert.AlertType.WARNING);
            return;
        }
        
        // Show confirmation dialog with file list
        showReceiptUploadConfirmation(record, selectedFiles, uploadedBy);
    }
}

private void showReceiptUploadConfirmation(PurchaseFundRecordModel record, List<File> files, String uploadedBy) {
    Dialog<ButtonType> dialog = new Dialog<>();
    dialog.setTitle("Confirm Receipt Upload");
    dialog.setHeaderText("Confirm Upload for Request: " + record.getRequestId());
    dialog.getDialogPane().setPrefSize(500, 400);

    VBox content = new VBox(15);
    content.setPadding(new Insets(20));

    Label titleLabel = new Label("Receipt Files to Upload:");
    ...
    for (File file : files) {
        String fileInfo = String.format("%s (%.2f KB)", 
            file.getName(), 
            file.length() / 1024.0
        );
    ...
    Label summaryLabel = new Label(String.format(
        "Total: %d files, %.2f MB", 
        files.size(), 
        files.stream().mapToLong(File::length).sum() / (1024.0 * 1024.0)
    ));
    summaryLabel.setStyle("-fx-font-weight: bold;");

    content.getChildren().addAll(titleLabel, fileListView, summaryLabel);
    ...
    if (result.isPresent() && result.get() == ButtonType.OK) {
        uploadReceiptFiles(record, files, uploadedBy);
    }
}

private void uploadReceiptFiles(PurchaseFundRecordModel record, List<File> files, String uploadedBy) {
    new Thread(() -> {
        try {
            Platform.runLater(() -> {
                operationStatusLabel.setText("Uploading receipt files...");
                operationStatusLabel.setStyle("-fx-text-fill: #f39c12;");
            });

            boolean success = databaseConnector.saveRecieptFiles(record.getRequestId(), files, uploadedBy);

            Platform.runLater(() -> {
                if (success) {
                    // Update the record in the table
                    record.setRecieptUploadStatus("Uploaded");
                    record.setRecieptUploadedBy(uploadedBy);
                    purchaseFundRequestTable.refresh();
                    updateButtonStates(record);
                    updateSummary();
                    
                    operationStatusLabel.setText("Receipt files uploaded successfully!");
                    operationStatusLabel.setStyle("-fx-text-fill: #27ae60;");
                    
                    showAlert("Upload Successful", 
                        String.format("%d receipt files uploaded successfully!\n\n" +
                                    "Request ID: %s\n" +
                                    "Uploaded by: %s\n" +
                                    "Files: %s", 
                            files.size(), 
                    ...
                    operationStatusLabel.setText("Receipt upload failed!");
                    operationStatusLabel.setStyle("-fx-text-fill: #e74c3c;");
                    showAlert("Upload Failed", "Failed to upload receipt files to database.", Alert.AlertType.ERROR);
                }
            });

        } catch (Exception e) {
            Platform.runLater(() -> {
                operationStatusLabel.setText("Upload error occurred!");
                operationStatusLabel.setStyle("-fx-text-fill: #e74c3c;");
                showAlert("Upload Error", "Error during receipt upload: " + e.getMessage(), Alert.AlertType.ERROR);
            });
        }
    }).start();
}
   ...
   private void voidSelected() {
    ...
    if (result.isPresent() && !result.get().isEmpty()) {
        ...
        boolean updated = databaseConnector.voidRecieptBasedPurchaseFundRequest(
    ...
    private void deleteSelected() {
        ...
        confirmation.setHeaderText("Delete Reciept Based Purchase Fund Request");
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            ...
            if (deleted) {
                purchaseFundRequestData.remove(selected);
                updateSummary();
    ...
    private void showRequestApprovalWithBiometricDialog(PurchaseFundRecordModel record) {
        Dialog<ButtonType> dialog = new Dialog<>();
        dialog.setTitle("Approve Request with Biometric Signature");
        dialog.setHeaderText("Approve Reciept Based Purchase Fund Request - " + record.getRequestId());
    ...
    private void showDispenseApprovalWithBiometricDialog(PurchaseFundRecordModel record) {
        Dialog<ButtonType> dialog = new Dialog<>();
        dialog.setTitle("Approve Dispense with Biometric Signature");
        dialog.setHeaderText("Approve Reciept Based Purchase Fund Dispense - " + record.getRequestId());
        dialog.getDialogPane().setPrefSize(700, 600);
    ...
    private void showConfirmationWithBiometricDialog(PurchaseFundRecordModel record) {
        Dialog<ButtonType> dialog = new Dialog<>();
        dialog.setTitle("Confirm Request with Biometric Signature");
        dialog.setHeaderText("Confirm Reciept Based Purchase Fund Request - " + record.getRequestId());
        dialog.getDialogPane().setPrefSize(700, 600);
...
private void showDispenseForm(PurchaseFundRecordModel record) {
    Dialog<ButtonType> dialog = new Dialog<>();
    ...
    // ✅ New fields for category info
    TextField mainCategoryField = new TextField(record.getMainCategory());
    mainCategoryField.setDisable(true);
    TextField subCategoryField = new TextField(record.getSubCategory());
    ...
    grid.addRow(2, new Label("Main Category:"), mainCategoryField);      // ✅ Added
    grid.addRow(3, new Label("Sub Category:"), subCategoryField);        // ✅ Added
    grid.addRow(4, new Label("Reason:"), reasonField);
    grid.addRow(5, new Label("Payee:"), payeeField);
    grid.addRow(6, new Label("Requested Amount:"), requestedAmountField);
    grid.addRow(7, new Label("Given Amount:"), givenAmountField);
    grid.addRow(8, new Label("Request Date:"), requestDateField);
    grid.addRow(9, new Label("Completed Date:"), completedDateField);
    grid.addRow(10, new Label("Given By:"), givenByField);
    ...
    if (result.isPresent() && result.get() == ButtonType.OK) {
       ...
       // ✅ Save category info along with dispense record
DispensedPurchaseFundModel dispensedRecord = new DispensedPurchaseFundModel(
    record.getRequestId(),
    record.getRequisitionUnit(),
    record.getMainCategory(),           // ✅ added
    record.getSubCategory(),            // ✅ added
    record.getReason(),
    verifiedPayee.getEmployeeName(),
    record.getAmountRequested(),        // ✅ now double, not String
    Double.parseDouble(givenAmountField.getText()), // ✅ converted to double
    ...
    "Dual Biometric Verified"
        ...
        // If your save method or table later supports categories, you can pass them like this:
        dispensedRecord.setMainCategory(record.getMainCategory());
        dispensedRecord.setSubCategory(record.getSubCategory());

        saveDispensedRecordWithDualSignatures(
            dispensedRecord,
            verifiedPayee.getSignatureImage(),
            verifiedDispenser.getSignatureImage(),
            payeeFingerprintRef.get(),
            dispenserFingerprintRef.get()
        );
    ...
    private void saveDispensedRecordWithDualSignatures(DispensedPurchaseFundModel record, 
                                                     byte[] payeeSignature, byte[] dispenserSignature,
                                                     byte[] payeeFingerprint, byte[] dispenserFingerprint) {
        new Thread(() -> {
            try {
                boolean success = databaseConnector.saveDispensedRecieptBasedPurchaseFundRecordWithDualSignatures(
                ...
                Platform.runLater(() -> {
                    if (success) {
                        ...
                        PurchaseFundRecordModel updatedRecord = findPurchaseFundRecordById(record.getRequestId());
    ...
    private PurchaseFundRecordModel findPurchaseFundRecordById(String requestId) {
        return purchaseFundRequestData.stream()
            .filter(record -> record.getRequestId().equals(requestId))
            .findFirst()
            .orElse(null);
    }

    private void saveRequestApprovalWithBiometric(PurchaseFundRecordModel record, EmployeeSignatureModel approver, 
                                         byte[] fingerprintTemplate, String notes) {
        new Thread(() -> {
            try {
                boolean success = databaseConnector.saveRecieptBasedPurchaseFundRequestApprovalWithBiometric(
    ...
    private void saveDispenseApprovalWithBiometric(PurchaseFundRecordModel record, EmployeeSignatureModel approver, 
                                         byte[] fingerprintTemplate, String notes) {
        new Thread(() -> {
            try {
                boolean success = databaseConnector.saveRecieptBasedPurchaseFundDispenseApprovalWithBiometric(
                    record, approver, fingerprintTemplate, notes
                );
    ...
    private void saveConfirmationWithBiometric(PurchaseFundRecordModel record, EmployeeSignatureModel confirmer, 
                                             byte[] fingerprintTemplate, String notes) {
        new Thread(() -> {
            try {
                boolean success = databaseConnector.saveRecieptBasedPurchaseFundConfirmationWithBiometric(
    ...
    private void updateButtonStates(PurchaseFundRecordModel selected) {
        if (selected == null) {
            ...
            uploadRecieptBtn.setDisable(true);
            return;
        }
        ...
        boolean isReceiptUploaded = "Uploaded".equalsIgnoreCase(selected.getRecieptUploadStatus());
        ...
        // Enable upload receipt button only for approved, not voided, and not already uploaded requests
        uploadRecieptBtn.setDisable(isVoided || !isApproved || isReceiptUploaded);
    ...
    private void updateButtonTooltips(PurchaseFundRecordModel selected) {
        ...
        if (isVoided) {
            ...
            uploadRecieptBtn.setTooltip(new Tooltip("Cannot upload receipts for voided request"));
        } else if (!isApproved) {
            ...
            uploadRecieptBtn.setTooltip(new Tooltip("Request must be approved before uploading receipts"));
        }
          else if (isReceiptUploaded) {
          uploadRecieptBtn.setTooltip(new Tooltip("Receipts already uploaded for this request"));
          }
        else if (!isDispensed) {
...
private void viewRecordDetails(PurchaseFundRecordModel record) {
    if (record == null) {
    ...
    try {
        ...
        dialog.setHeaderText(null); // custom header
        dialog.getDialogPane().setPrefSize(1200, 900);
        dialog.setResizable(true);
       ...
       // Center: Hospital Name & Info
        ...
        Label title = new Label("RECIEPT BASED PURCHASE FUNG REPORT");
        ...
        basicInfoGrid.addRow(3,
            createBoldLabel("Main Category:"), new Label(getSafeString(record.getMainCategory())),
            createBoldLabel("Sub Category:"), new Label(getSafeString(record.getSubCategory()))
        );
        ...
        statusGrid.addRow(5,
            createBoldLabel("Void Reason:"), createStatusLabel(getSafeString(databaseConnector.getRecieptBasedPurchaseFundRequestVoidReason(record.getRequestId())))
        );
        statusGrid.addRow(6,
            createBoldLabel("Receipt Upload Status:"), createStatusLabel(getSafeString(record.getRecieptUploadStatus())),
            createBoldLabel("Receipt Uploaded By:"), new Label(getSafeString(record.getRecieptUploadedBy()))
        );
        ...
        // ===== RECEIPTS SECTION =====
        VBox receiptsSection = createDetailSection("UPLOADED RECEIPTS", "#16a085");
        VBox receiptsContainer = new VBox(15);
        receiptsContainer.setPadding(new Insets(15));
        loadReceiptsForDetails(record, receiptsContainer); // async load
        receiptsSection.getChildren().add(receiptsContainer);

        // Add all main sections
        mainContent.getChildren().addAll(basicInfoSection, statusSection, signaturesSection,receiptsSection);
...
exportButton.setOnAction(e -> {
    ...
    // Temporarily remove receipts section from mainContent
    mainContent.getChildren().remove(receiptsSection);
    topButtons.getChildren().remove(exportButton);

    // Export without receipts
    exportNodeToPDF(contentWithButtons, "PurchaseFundRequest_" + record.getRequestId() + ".pdf", record.getRequestId());

    // Add receipts section back after a small delay to ensure UI updates
    Platform.runLater(() -> {
        if (!mainContent.getChildren().contains(receiptsSection)) {
            mainContent.getChildren().add(receiptsSection);
        ...
        // Enable/disable export based on signature presence
        new Thread(() -> {
...
// ==================== RECEIPTS DISPLAY METHODS ====================

private void loadReceiptsForDetails(PurchaseFundRecordModel record, VBox receiptsContainer) {
    if (!"Uploaded".equalsIgnoreCase(record.getRecieptUploadStatus())) {
        Label noReceiptsLabel = new Label("No receipts uploaded for this request");
        noReceiptsLabel.setStyle("-fx-text-fill: #7f8c8d; -fx-font-style: italic; -fx-font-size: 14;");
        receiptsContainer.getChildren().add(noReceiptsLabel);
        return;
    }
    ...
    receiptsContainer.getChildren().add(loadingIndicator);

    new Thread(() -> {
        try {
            List<String> receiptFileNames = databaseConnector.getRecieptFileNames(record.getRequestId());
            List<byte[]> receiptFiles = databaseConnector.getRecieptFiles(record.getRequestId());

            Platform.runLater(() -> {
                receiptsContainer.getChildren().clear();

                if (receiptFileNames.isEmpty() || receiptFiles.isEmpty()) {
                    Label noReceiptsLabel = new Label("No receipt files found in database");
                    noReceiptsLabel.setStyle("-fx-text-fill: #e74c3c; -fx-font-style: italic; -fx-font-size: 14;");
                    receiptsContainer.getChildren().add(noReceiptsLabel);
                    return;
                }

                // Receipts summary
                Label summaryLabel = new Label(String.format("📎 %d receipt file(s) uploaded", receiptFileNames.size()));
                summaryLabel.setStyle("-fx-font-size: 16; -fx-font-weight: bold; -fx-text-fill: #2c3e50;");
                receiptsContainer.getChildren().add(summaryLabel);

                // Create a grid for receipts
                GridPane receiptsGrid = new GridPane();
                receiptsGrid.setHgap(20);
                receiptsGrid.setVgap(20);
                receiptsGrid.setPadding(new Insets(15, 0, 15, 0));

                int col = 0;
                int row = 0;
                int maxCols = 2; // 2 receipts per row

                for (int i = 0; i < receiptFiles.size(); i++) {
                    String fileName = receiptFileNames.get(i);
                    byte[] fileData = receiptFiles.get(i);

                    // Pass requestId to createReceiptCard for auto-download
                    VBox receiptCard = createReceiptCard(fileName, fileData, i + 1, record.getRequestId());
                    receiptsGrid.add(receiptCard, col, row);
                ...
                receiptsContainer.getChildren().add(receiptsGrid);
            });
        } catch (Exception e) {
            Platform.runLater(() -> {
                receiptsContainer.getChildren().clear();
                Label errorLabel = new Label("❌ Error loading receipts: " + e.getMessage());
                errorLabel.setStyle("-fx-text-fill: #e74c3c; -fx-font-size: 14;");
                receiptsContainer.getChildren().add(errorLabel);
            });
        }
    }).start();
}
...
private VBox createReceiptCard(String fileName, byte[] fileData, int index, String requestId) {
    ...
    receiptCard.setPrefWidth(350);
    receiptCard.setMaxWidth(350);
    ...
    Label fileIndex = new Label("Receipt #" + index);
    fileIndex.setStyle("-fx-font-weight: bold; -fx-text-fill: #2c3e50; -fx-font-size: 12;");
    Label fileNameLabel = new Label(truncateFileName(fileName, 30));
    fileNameLabel.setStyle("-fx-text-fill: #34495e; -fx-font-size: 14; -fx-font-weight: bold;");
    Label fileSize = new Label(getFileSize(fileData));
    fileSize.setStyle("-fx-text-fill: #7f8c8d; -fx-font-size: 11;");

    fileInfo.getChildren().addAll(fileIndex, fileNameLabel, fileSize);
    headerBox.getChildren().addAll(iconLabel, fileInfo);
    ...
    Node previewContent = createFilePreview(fileName, fileData);
    previewPane.getChildren().add(previewContent);

    // ===== Buttons =====
    HBox buttonBox = new HBox(10);
    buttonBox.setAlignment(Pos.CENTER);

    Button viewBtn = createSmallButton("👁️ View", "#3498db");
viewBtn.setOnAction(e -> viewReceiptFile(fileName, fileData, requestId));

    Button downloadBtn = createSmallButton("📥 Download", "#27ae60");
    downloadBtn.setOnAction(e -> {
        // Auto-download to Documents/Receipts/<Request_ID>/
        downloadReceiptFile(fileName, fileData, requestId);
    });

    buttonBox.getChildren().addAll(viewBtn, downloadBtn);

    receiptCard.getChildren().addAll(headerBox, previewPane, buttonBox);
    return receiptCard;
}


private Node createFilePreview(String fileName, byte[] fileData) {
    ...
    try {
        switch (fileExtension) {
            ...
            case "gif":
                // Image preview
                Image image = convertByteArrayToImage(fileData);
            ...
            case "doc":
            case "docx":
                return createWordPreview();

            case "xls":
            case "xlsx":
                return createExcelPreview();

            default:
                return createGenericFilePreview(fileExtension);
        }
    } catch (Exception e) {
...
private Node createPdfPreview() {
    ...
    Label pdfText = new Label("PDF Document");
    pdfText.setStyle("-fx-text-fill: #e74c3c; -fx-font-weight: bold;");
    
    pdfPreview.getChildren().addAll(pdfIcon, pdfText);
    return pdfPreview;
}

private Node createWordPreview() {
    VBox wordPreview = new VBox(10);
    wordPreview.setAlignment(Pos.CENTER);
    
    Label wordIcon = new Label("📝");
    wordIcon.setStyle("-fx-font-size: 40;");
    
    Label wordText = new Label("Word Document");
    wordText.setStyle("-fx-text-fill: #2980b9; -fx-font-weight: bold;");
    
    wordPreview.getChildren().addAll(wordIcon, wordText);
    return wordPreview;
}

private Node createExcelPreview() {
    VBox excelPreview = new VBox(10);
    excelPreview.setAlignment(Pos.CENTER);
    
    Label excelIcon = new Label("📊");
    excelIcon.setStyle("-fx-font-size: 40;");
    
    Label excelText = new Label("Excel Spreadsheet");
    excelText.setStyle("-fx-text-fill: #27ae60; -fx-font-weight: bold;");
    
    excelPreview.getChildren().addAll(excelIcon, excelText);
    return excelPreview;
}

private Node createGenericFilePreview(String fileExtension) {
    VBox genericPreview = new VBox(10);
    genericPreview.setAlignment(Pos.CENTER);
    
    Label fileIcon = new Label("📎");
    fileIcon.setStyle("-fx-font-size: 40;");
    
    Label fileText = new Label(fileExtension.toUpperCase() + " File");
    fileText.setStyle("-fx-text-fill: #7f8c8d; -fx-font-weight: bold;");
    
    genericPreview.getChildren().addAll(fileIcon, fileText);
    return genericPreview;
}

private Node createErrorPreview() {
    VBox errorPreview = new VBox(10);
    errorPreview.setAlignment(Pos.CENTER);
    
    Label errorIcon = new Label("❌");
    errorIcon.setStyle("-fx-font-size: 40;");
    
    Label errorText = new Label("Preview Unavailable");
    errorText.setStyle("-fx-text-fill: #e74c3c; -fx-font-size: 12;");
    
    errorPreview.getChildren().addAll(errorIcon, errorText);
    return errorPreview;
}
...
private String truncateFileName(String fileName, int maxLength) {
    ...
    if (name.length() > maxLength - 7) {
        return name.substring(0, maxLength - 7) + "..." + extension;
    }
    
    return fileName;
}

private String getFileSize(byte[] fileData) {
...
private void viewReceiptFile(String fileName, byte[] fileData, String requestId) {
    try {
        String extension = getFileExtension(fileName).toLowerCase();
        ...
        switch (extension) {
            ...
            case "png":
            case "gif":
                showImagePreview(fileName, fileData);
                break;
            case "pdf":
                showPdfPreview(fileName, fileData);
                break;
            default:
                // For other file types, offer download
                if (showConfirmation("View File", 
                    "This file type cannot be previewed directly. Would you like to download it instead?")) {
                    downloadReceiptFile(fileName, fileData, requestId); // Pass requestId here
                }
                break;
        }
    } catch (Exception e) {
        showAlert("View Error", "Cannot preview file: " + e.getMessage(), Alert.AlertType.ERROR);
    }
}


private void showImagePreview(String fileName, byte[] fileData) {
    Dialog<Void> dialog = new Dialog<>();
...
private void showPdfPreview(String fileName, byte[] fileData) {
    // For PDF preview, we'll save temporarily and open with system default
    try {
        ...
        if (Desktop.isDesktopSupported()) {
            ...
            showAlert("PDF Preview", 
                "PDF file saved to: " + tempFile.getAbsolutePath() + 
                "\nPlease open it with your PDF viewer.", 
                Alert.AlertType.INFORMATION);
        }
    } catch (Exception e) {
        showAlert("PDF Error", "Cannot open PDF: " + e.getMessage(), Alert.AlertType.ERROR);
    }
}

private void downloadReceiptFile(String fileName, byte[] fileData, String requestId) {
    try {
        // Folder: Documents/Receipts/<Request_ID>/
        File outputDir = new File(System.getProperty("user.home"), "Documents/Receipts/" + requestId);
        ...
        e.printStackTrace();
        showAlert("Download Failed", "Failed to download file: " + e.getMessage(), Alert.AlertType.ERROR);
    }
}
        ...
        if (proceed) {
            ...
            if (success) {
                ...
                showAlert("Print Successful", 
                    "Reciept Based Purchase Fund request details were sent to the printer.", 
                    Alert.AlertType.INFORMATION);
            } else {
...
private void exportNodeToPDF(Node node, String fileName, String requestId) {
    try {
        ...
        Platform.runLater(() -> {
            try {
                ...
                // Create folder: Documents/Receipts/<Request_ID>/
                File outputDir = new File(System.getProperty("user.home"), "Documents/ReceiptsBasedPurchaseFund/" + requestId);
                if (!outputDir.exists()) outputDir.mkdirs();
...
private void loadSignaturesForDetails(PurchaseFundRecordModel record, HBox signaturesContainer) {
...
private String getSignatureTitleByRole(String role) {
    ...
    private boolean hasAllSignatures(PurchaseFundRecordModel record) {
    try {
    ...
    private void printDateRangeTabular(LocalDate fromDate, LocalDate toDate) {
        List<PurchaseFundRecordModel> filteredData = purchaseFundRequestData.stream()
    ...
    private void exportSelectedRowToExcelWithImages() {
        ...
        fileChooser.setInitialFileName("Reciept_Based_Purchase_Fund_Details_" + selected.getRequestId() + "_" + LocalDate.now() + ".xlsx");
        ...
        if (file != null) {
            new Thread(() -> {
                try (Workbook workbook = new XSSFWorkbook()) {
                    ...
                    headerCell.setCellValue("AFRAN GENERAL HOSPITAL - RECIEPT BASED PURCHASE FUND REQUEST DETAILS");
    ...
    private void exportDateRangeToExcel(LocalDate fromDate, LocalDate toDate) {
        ...
        fileChooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("Excel Files", "*.xlsx"));
        fileChooser.setInitialFileName("Reciept_Based_Purchase_Fund_Report_" + fromDate + "_to_" + toDate + ".xlsx");
    ...
    private void exportSelectedRowToWordWithImages() {
        ...
        fileChooser.setInitialFileName("Reciept_Based_Purchase_Fund_" + selected.getRequestId() + "_" + LocalDate.now() + ".docx");
        ...
        if (file != null) {
            new Thread(() -> {
                try (XWPFDocument document = new XWPFDocument()) {
                    ...
                    subtitleRun.setText("RECIEPT BASED PURCHASE FUND REQUEST DETAILS");
    ...
    private void exportDateRangeToWord(LocalDate fromDate, LocalDate toDate) {
        ...
        fileChooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("Word Documents", "*.docx"));
        fileChooser.setInitialFileName("Reciept Based Purchase Fund_" + fromDate + "_to_" + toDate + ".docx");
    ...
    private void exportDateRangeToWordWithImages(LocalDate fromDate, LocalDate toDate) {
        ...
        fileChooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("Word Documents", "*.docx"));
        fileChooser.setInitialFileName("Reciept_Based_Purchase_Fund_" + fromDate + "_to_" + toDate + ".docx");
    ...
    private VBox createDetailedPrintNode(PurchaseFundRecordModel record, String title) {
        ...
        Label subHeader = new Label("RECIEPT BASED PURCHASE FUND REQUEST MANAGEMENT SYSTEM");
    ...
    private VBox createTabularPrintNode(List<PurchaseFundRecordModel> data, LocalDate fromDate, LocalDate toDate) {
        VBox printNode = new VBox(15);
        ...
        Label subHeader = new Label("RECIEPT BASED PURCHASE FUND REQUESTS SUMMARY REPORT");
        ...
        TableColumn<PurchaseFundRecordModel, String> reqIdCol = new TableColumn<>("Request ID");
        ...
        TableColumn<PurchaseFundRecordModel, Double> amountCol = new TableColumn<>("Amount");
    ...
    private VBox createDateRangeDetailedPrintNode(List<PurchaseFundRecordModel> data, LocalDate fromDate, LocalDate toDate) {
        VBox printNode = new VBox(15);
        ...
        Label subHeader = new Label("DETAILED RECIEPT PURCHSE FUND REQUESTS REPORT");
        subHeader.setStyle("-fx-font-size: 14; -fx-font-weight: bold; -fx-text-fill: #3498db; -fx-alignment: center;");
        ...
        for (int i = 0; i < Math.min(data.size(), 5); i++) { // Limit to 5 records for printing
            PurchaseFundRecordModel record = data.get(i);
    ...
    private VBox createRecordPrintBox(PurchaseFundRecordModel record, int index) {
    ...
    private void exportToExcelWithData(List<PurchaseFundRecordModel> data, File file, LocalDate fromDate, LocalDate toDate) {
        new Thread(() -> {
            try (Workbook workbook = new XSSFWorkbook()) {
                Sheet sheet = workbook.createSheet("Reciept Based Purchase Fund Requests");
                ...
                titleCell.setCellValue("AFRAN GENERAL HOSPITAL - RECIEPT BASED PURCHASE FUND REQUESTS REPORT");
                ...
                // Data rows
                int rowNum = 4;
                for (PurchaseFundRecordModel record : data) {
                    Row row = sheet.createRow(rowNum++);
                ...
                summaryCell.setCellValue("SUMMARY: Total Requests: " + data.size() + 
                    " | Total Amount: ETB " + data.stream().mapToDouble(PurchaseFundRecordModel::getAmountRequested).sum());
    ...
    private void exportToExcelWithImages(List<PurchaseFundRecordModel> data, File file, LocalDate fromDate, LocalDate toDate) {
    ...
    private void exportToWordWithData(List<PurchaseFundRecordModel> data, File file, LocalDate fromDate, LocalDate toDate) {
        new Thread(() -> {
            try (XWPFDocument document = new XWPFDocument()) {
                // Title
                XWPFParagraph titleParagraph = document.createParagraph();
                ...
                subtitleParagraph.setAlignment(ParagraphAlignment.CENTER);
                XWPFRun subtitleRun = subtitleParagraph.createRun();
                subtitleRun.setText("RECIEPT BASED PURCHASE FUND REQUESTS REPORT");
                subtitleRun.setBold(true);
                subtitleRun.setFontSize(14);
                ...
                summaryRun.setText("Summary: Total Requests: " + data.size() + 
                    ...
                    data.stream().mapToDouble(PurchaseFundRecordModel::getAmountRequested).sum()));
    ...
    private void exportToWordWithImages(List<PurchaseFundRecordModel> data, File file, LocalDate fromDate, LocalDate toDate) {
    ...
    private VBox createRequestDetailsSection(PurchaseFundRecordModel record) {
...
dateToPicker.getEditor().setPromptText("yyyy-MM-dd");
       ...
       filterBox.getChildren().addAll(
        ...
        purchaseFundRequestTable = new TableView<>();
        purchaseFundRequestTable.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY);
        purchaseFundRequestTable.setStyle("-fx-background-color: #ecf0f1; -fx-border-color: #bdc3c7; -fx-border-radius: 5;");

        // Proper row factory for double-click functionality
       purchaseFundRequestTable.setRowFactory(tv -> {
    TableRow<PurchaseFundRecordModel> row = new TableRow<PurchaseFundRecordModel>() {
        @Override
        protected void updateItem(PurchaseFundRecordModel item, boolean empty) {
    ...
    row.setOnMouseClicked(event -> {
        if (event.getClickCount() == 2 && !row.isEmpty()) {
            PurchaseFundRecordModel selectedRecord = row.getItem();
    ...
    return row;
        ...
        TableColumn<PurchaseFundRecordModel, String> requestIdCol = createStyledColumn("Request ID", "requestId", "#2c3e50");
        TableColumn<PurchaseFundRecordModel, String> requisitionUnitCol = createStyledColumn("Requisition Unit", "requisitionUnit", "#34495e");
        TableColumn<PurchaseFundRecordModel, String> reasonCol = createStyledColumn("Reason", "reason", "#16a085");
        TableColumn<PurchaseFundRecordModel, String> payeeCol = createStyledColumn("Payee", "payee", "#27ae60");
        TableColumn<PurchaseFundRecordModel, Double> amountRequestedCol = createStyledColumn("Amount Requested", "amountRequested", "#2980b9");
        TableColumn<PurchaseFundRecordModel, LocalDate> requestDateCol = createStyledColumn("Request Date", "requestDate", "#8e44ad");
        TableColumn<PurchaseFundRecordModel, String> approvalStatusCol = createStyledColumn("Approval Status", "approvalStatus", "#c0392b");
        TableColumn<PurchaseFundRecordModel, String> approvedByCol = createStyledColumn("Approved By", "approvedBy", "#7f8c8d");
        TableColumn<PurchaseFundRecordModel, String> confirmationStatusCol = createStyledColumn("Confirmation Status", "confirmationStatus", "#f39c12");
        TableColumn<PurchaseFundRecordModel, String> confirmedByCol = createStyledColumn("Confirmed By", "confirmedBy", "#d35400");
        TableColumn<PurchaseFundRecordModel, String> dispensedStatusCol = createStyledColumn("Dispensed Status", "dispensedStatus", "#16a085");
        TableColumn<PurchaseFundRecordModel, String> dispensedByCol = createStyledColumn("Dispensed By", "dispensedBy", "#27ae60");
        TableColumn<PurchaseFundRecordModel, String> dispenseApprovalStatusCol = createStyledColumn("Dispense Approval Status", "dispenseApprovalStatus", "#d35400");
        TableColumn<PurchaseFundRecordModel, String> dispenseApprovedByCol = createStyledColumn("Dispense Approved By", "dispenseApprovedBy", "#8e44ad");
        TableColumn<PurchaseFundRecordModel, String> voidStatusCol = createStyledColumn("Void Status", "voidStatus", "#e74c3c");
        TableColumn<PurchaseFundRecordModel, String> voidedByCol = createStyledColumn("Voided By", "voidedBy", "#c0392b");
        TableColumn<PurchaseFundRecordModel, String> recieptUploadStatusCol = createStyledColumn("Reciept Upload Status", "recieptUploadStatus", "#e32c3c");
        TableColumn<PurchaseFundRecordModel, String> recieptUploadedByCol = createStyledColumn("Reciept Uploaded By", "recieptUploadedBy", "#c4392b");
        TableColumn<PurchaseFundRecordModel, String> mainCategoyrCol = createStyledColumn("Main Category Column", "mainCategory", "#e32c3c");
        TableColumn<PurchaseFundRecordModel, String> subcategoryByCol = createStyledColumn("Subcategory Column", "subCategory", "#c4392b");

        purchaseFundRequestTable.getColumns().addAll(
            ...
            dispenseApprovedByCol, voidStatusCol, voidedByCol,recieptUploadStatusCol,
            recieptUploadedByCol,mainCategoyrCol,subcategoryByCol
        );

        purchaseFundRequestTable.setItems(purchaseFundRequestData);
        
        Label placeholder = new Label("No Reciept Based Purchase Fund requests found. Click 'New Request' to create one.");
        placeholder.setStyle("-fx-text-fill: #7f8c8d; -fx-font-size: 14; -fx-padding: 20;");
        purchaseFundRequestTable.setPlaceholder(placeholder);

        // Proper selection listener
        purchaseFundRequestTable.getSelectionModel().selectedItemProperty().addListener((obs, oldSel, newSel) -> {
            if (newSel != null) {
        ...
        tableSection.getChildren().addAll(filterBox, purchaseFundRequestTable);
        return tableSection;
    }


      private <T> TableColumn<PurchaseFundRecordModel, T> createStyledColumn(String title, String property, String color) {
    TableColumn<PurchaseFundRecordModel, T> column = new TableColumn<>(title);
    column.setCellValueFactory(new PropertyValueFactory<>(property));
    column.setStyle("-fx-background-color: " + color + "; -fx-text-fill: white; -fx-font-weight: bold;");
    
    column.setCellFactory(tc -> new TableCell<PurchaseFundRecordModel, T>() {
        @Override
    ...
    purchaseFundRequestTable.setItems(
    purchaseFundRequestData.filtered(record -> {
    ...
    private void refreshData() {
        ...
        showAlert("Refreshed", "Data refreshed successfully!\n" + 
                  "Loaded " + purchaseFundRequestData.size() + " Reciept Based Purchase Fund requests and " + 
    ...
    private void showAdvancedSearchDialog() {
        ...
        dialog.setHeaderText("Search Reciept Based Purchase Fund Requests with Multiple Criteria");
        dialog.getDialogPane().setPrefSize(600, 500);
    ...
    private void performAdvancedSearch(String requestId, String payee, String requisitionUnit, 
        ...
        purchaseFundRequestTable.setItems(purchaseFundRequestData.filtered(record -> {
        ...
        showAlert("Search Complete", 
            "Found " + purchaseFundRequestTable.getItems().size() + " matching records.",
            Alert.AlertType.INFORMATION);
    }

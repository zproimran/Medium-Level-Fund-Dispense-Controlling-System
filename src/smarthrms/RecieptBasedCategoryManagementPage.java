...
public class RecieptBasedCategoryManagementPage extends BorderPane {
    ...
    public RecieptBasedCategoryManagementPage(String username) {
        ...
        Label header = new Label("Receipt Based Purchase Fund Category Management");
    ...
    private VBox createMainCategoryBox() {
        ...
        Label title = new Label("Receipt Based Purchase Main Categories");
    ...
    private void loadMainCategories() {
        ObservableList<String> list = categoryDAO.getMainCategories();
    ...
    private void addMainCategory() {
        ...
        dialog.setTitle("Add Receipt Based Purchase Main Category");
        dialog.setHeaderText("Enter new receipt based purchase main category:");
        ...
        dialog.showAndWait().ifPresent(name -> {
            ...
            if (categoryDAO.addMainCategory(name.trim())) {
                loadMainCategories();
    ...
    private void editMainCategory() {
        ...
        if (selected == null) { 
            showWarning("Select a receipt based purchase main category to edit."); 
            return; 
        }

        TextInputDialog dialog = new TextInputDialog(selected);
        dialog.setTitle("Edit Receipt Based Purchase Main Category");
        dialog.setHeaderText("Edit receipt based purchase main category name:");
        ...
        dialog.showAndWait().ifPresent(newName -> {
            ...
            if (categoryDAO.updateMainCategory(selected, newName.trim())) {
                loadMainCategories();
    ...
    private void deleteMainCategory() {
        ...
        String selected = mainCategoryTable.getSelectionModel().getSelectedItem();
        if (selected == null) { 
            showWarning("Select a receipt based purchase main category to delete."); 
            return; 
        }

        // Check if category has subcategories
        ObservableList<String> subCategories = categoryDAO.getSubCategories(selected);
        ...
        confirmation.showAndWait().ifPresent(result -> {
            if (result == ButtonType.OK) {
                if (categoryDAO.deleteMainCategory(selected)) {
                    loadMainCategories();
    ...
    private VBox createSubCategoryBox() {
        ...
        Label title = new Label("Receipt Based Purchase Subcategories");
        title.setStyle("-fx-font-size: 16px; -fx-font-weight: bold;");
    ...
    private void loadSubCategories() {
        ...
        subCategoryTable.setItems(categoryDAO.getSubCategories(selectedMainCategory));
    }

    private void addSubCategory() {
        if (selectedMainCategory == null) { 
            showWarning("Select a receipt based purchase main category first."); 
            return; 
        }

        TextInputDialog dialog = new TextInputDialog();
        dialog.setTitle("Add Receipt Based Purchase Subcategory");
        dialog.setHeaderText("Add receipt based purchase subcategory under: " + selectedMainCategory);
        ...
        dialog.showAndWait().ifPresent(name -> {
            ...
            if (categoryDAO.addSubCategory(selectedMainCategory, name.trim())) {
                loadSubCategories();
    ...
    private void editSubCategory() {
        ...
        if (selected == null) { 
            showWarning("Select a receipt based purchase subcategory to edit."); 
            return; 
        }

        TextInputDialog dialog = new TextInputDialog(selected);
        dialog.setTitle("Edit Receipt Based Purchase Subcategory");
        ...
        dialog.showAndWait().ifPresent(newName -> {
            ...
            if (categoryDAO.updateSubCategory(selectedMainCategory, selected, newName.trim())) {
                loadSubCategories();
    ...
    private void deleteSubCategory() {
        ...
        String selected = subCategoryTable.getSelectionModel().getSelectedItem();
        if (selected == null) { 
            showWarning("Select a receipt based purchase subcategory to delete."); 
            return; 
        }
        ...
        confirmation.showAndWait().ifPresent(result -> {
            if (result == ButtonType.OK) {
                if (categoryDAO.deleteSubCategory(selectedMainCategory, selected)) {
                    loadSubCategories();

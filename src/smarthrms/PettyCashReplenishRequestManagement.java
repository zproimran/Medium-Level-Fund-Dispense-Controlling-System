...
public class PettyCashReplenishRequestManagement extends BorderPane { 
    
    private String currentUser;

    private TableView<PettyCashReplenishRecordModel> pettyCashReplenishRequestTable;
    private ObservableList<PettyCashReplenishRecordModel> pettyCashReplenishRequestData = FXCollections.observableArrayList();
    ...
    public PettyCashReplenishRequestManagement(String username) {
        ...
        loadPettyCashRequestsFromDatabase();
    ...
    private VBox createHeader() {
        ...
        Text departmentName = new Text("PETTY CASH REPLENISH REQUEST MANAGEMENT");
    ...
    private HBox createFooter() {
        ...
        Text footerText = new Text("© 2025 Afran General Hospital - Smart HRMS | Petty Cash Replenish Management System v3.0 | Biometric Signature Integration");
    ...
    private void updateSummary() {
        if (pettyCashReplenishRequestData.isEmpty()) {
            summaryLabel.setText("No petty cash replenish requests found");
            return;
        }

        long totalRequests = pettyCashReplenishRequestData.size();
        long pendingApproval = pettyCashReplenishRequestData.stream()
            .filter(r -> "Pending".equals(r.getApprovalStatus()))
            .count();
        long approved = pettyCashReplenishRequestData.stream()
            .filter(r -> "Approved".equals(r.getApprovalStatus()))
            .count();
        long confirmed = pettyCashReplenishRequestData.stream()
            .filter(r -> "Confirmed".equals(r.getConfirmationStatus()))
            .count();
        long dispensed = pettyCashReplenishRequestData.stream()
            .filter(r -> "Yes".equals(r.getDispensedStatus()))
            .count();
        double totalAmount = pettyCashReplenishRequestData.stream()
            .mapToDouble(PettyCashReplenishRecordModel::getAmountRequested)
            .sum();
    ...
    private VBox createButtonPanel() {
        ...
        requestBtn.setOnAction(e -> showPettyCashRequestForm());
        ...
        dispenseBtn.setOnAction(e -> {
            PettyCashReplenishRecordModel selected = pettyCashReplenishRequestTable.getSelectionModel().getSelectedItem();
            if (selected != null) {
                showDispenseForm(selected);
            } else {
                showAlert("No Selection", "Please select a petty cash request to dispense.", Alert.AlertType.WARNING);
            }
        });
    ...
    private void loadPettyCashRequestsFromDatabase() {
        new Thread(() -> {
            try {
                List<PettyCashReplenishRecordModel> requests = databaseConnector.getAllPettyCashReplenishRequests();
                Platform.runLater(() -> {
                    pettyCashReplenishRequestData.clear();
                    pettyCashReplenishRequestData.addAll(requests);
                    System.out.println("Loaded " + requests.size() + " petty cash requests from database");
                    updateSummary();
                    
                    if (!pettyCashReplenishRequestData.isEmpty()) {
                        pettyCashReplenishRequestTable.getSelectionModel().selectFirst();
                        updateButtonStates(pettyCashReplenishRequestTable.getSelectionModel().getSelectedItem());
                    }
                });
            } catch (Exception e) {
                Platform.runLater(() -> 
                    showAlert("Database Error", "Failed to load petty cash requests: " + e.getMessage(), Alert.AlertType.ERROR));
            }
        }).start();
    }
...
private void showPettyCashRequestForm() {
    Dialog<PettyCashReplenishRecordModel> dialog = new Dialog<>();
    dialog.setTitle("New Petty Cash Request");
    dialog.setHeaderText("Create New Petty Cash Request for Afran General Hospital");
    ...
    reasonField.setPromptText("Enter reason for petty cash request...");
    ...
    dialog.setResultConverter(dialogButton -> {
        if (dialogButton == ButtonType.OK) {
            ...
            return new PettyCashReplenishRecordModel(
                requestId,
    ...
    Optional<PettyCashReplenishRecordModel> result = dialog.showAndWait();
    result.ifPresent(newRecord -> {
        boolean saved = databaseConnector.savePettyCashReplenishRequest(newRecord);
        if (saved) {
            pettyCashReplenishRequestData.add(0, newRecord);
            pettyCashReplenishRequestTable.getSelectionModel().select(newRecord);
            updateSummary();
            showAlert("Success", "New petty cash request created successfully!\nRequest ID: " + newRecord.getRequestId(), Alert.AlertType.INFORMATION);
        } else {
...
private String generateRequestId() {
    String prefix = "PCREPLN"; // Petty Cash Replenish prefix
    DateTimeFormatter dateFormatter = DateTimeFormatter.ofPattern("yyyyMMdd");
    String datePart = LocalDate.now().format(dateFormatter);
    
    int maxNumber = pettyCashReplenishRequestData.stream()
    ...
    // Examples: PCREPLN20231215001, PCREPLN20231215002
}
...
//        dialog.setTitle("Edit Petty Cash Request");
...
//                
//                boolean updated = databaseConnector.updatePettyCashRequest(selected);
//                if (updated) {
//                    pettyCashReplenishRequestTable.refresh();
//                    updateSummary();
...
//        confirmation.setHeaderText("Delete Petty Cash Request");
...
//            boolean deleted = databaseConnector.deletePettyCashRequest(selected.getRequestId());
//            if (deleted) {
//                pettyCashReplenishRequestData.remove(selected);
//                updateSummary();
...
private void voidSelected() {
    ...
    dialog.setTitle("Void Petty Cash Request");
    ...
    if (result.isPresent() && !result.get().isEmpty()) {
        ...
        boolean updated = databaseConnector.voidPettyCashReplenishRequest(
    ...
    // Check if there's already an active petty cash replenishment fund in use
    if (databaseConnector.hasActivePettyCashReplenishmentFundInUse()) {
        showAlert("Active Fund in Use", 
            "Cannot approve new petty cash dispense while there is an active petty cash fund in use.\n\n" +
            "Please wait until the current active petty cash fund is fully utilized or closed before approving a new dispense.",
            Alert.AlertType.ERROR);
        return;
    }
    ...
    private void showRequestApprovalWithBiometricDialog(PettyCashReplenishRecordModel record) {
        Dialog<ButtonType> dialog = new Dialog<>();
        dialog.setTitle("Approve Request with Biometric Signature");
        dialog.setHeaderText("Approve Petty Cash Request - " + record.getRequestId());
    ...
    private void showDispenseApprovalWithBiometricDialog(PettyCashReplenishRecordModel record) {
        Dialog<ButtonType> dialog = new Dialog<>();
        dialog.setTitle("Approve Dispense with Biometric Signature");
        dialog.setHeaderText("Approve Petty Cash Dispense - " + record.getRequestId());
        dialog.getDialogPane().setPrefSize(700, 600);
    ...
    private void showConfirmationWithBiometricDialog(PettyCashReplenishRecordModel record) {
        Dialog<ButtonType> dialog = new Dialog<>();
        dialog.setTitle("Confirm Request with Biometric Signature");
        dialog.setHeaderText("Confirm Petty Cash Request - " + record.getRequestId());
        dialog.getDialogPane().setPrefSize(700, 600);
    ...
    private void showDispenseForm(PettyCashReplenishRecordModel record) {
    // First check if there's already an active petty cash replenishment fund in use
    if (databaseConnector.hasActivePettyCashReplenishmentFundInUse()) {
        showAlert("Active Fund in Use", 
            "Cannot dispense new petty cash replenishment fund while there is an active fund in use.\n\n" +
            "Please wait until the current active petty cash fund is fully utilized or closed before dispensing a new replenishment.",
            Alert.AlertType.ERROR);
        return;
    }
    ...
    VBox cashierVerificationSection = createBiometricSection(
        "Cashier Identity Verification", 
        "Step 1: Verify cashier identity using fingerprint",
        "#3498db"
    );

    VBox dispenserVerificationSection = createBiometricSection(
        "Replenish Dispenser Identity Verification",
        "Step 2: Verify dispenser identity using fingerprint", 
        "#9b59b6"
    );

    HBox dualBiometricContainer = new HBox(20);
    dualBiometricContainer.getChildren().addAll(cashierVerificationSection, dispenserVerificationSection);
    ...
    Button payeeVerifyBtn = (Button) cashierVerificationSection.getChildren().get(1);
    ImageView payeeSignatureView = (ImageView) cashierVerificationSection.getChildren().get(3);
    Label payeeStatusLabel = (Label) cashierVerificationSection.getChildren().get(4);
    ...
    if (result.isPresent() && result.get() == ButtonType.OK) {
        ...
        DispensedPettyCashModel dispensedRecord = new DispensedPettyCashModel(
            record.getRequestId(),
        ...
        showAlert("Dispense Complete",
            "Replenish dispensed successfully with dual biometric verification!\n\n" +
            "Request ID: " + record.getRequestId() + "\n" +
    ...
    private void saveDispensedRecordWithDualSignatures(DispensedPettyCashModel record, 
                                                     byte[] payeeSignature, byte[] dispenserSignature,
                                                     byte[] payeeFingerprint, byte[] dispenserFingerprint) {
        new Thread(() -> {
            try {
                boolean success = databaseConnector.saveDispensedPettyCashReplenishRecordWithDualSignatures(
                ...
                Platform.runLater(() -> {
                    if (success) {
                        ...
                        // Find and update the corresponding PettyCashReplenishRecordModel
                        PettyCashReplenishRecordModel updatedRecord = findPettyCashRecordById(record.getRequestId());
    ...
    // Helper method to find PettyCashReplenishRecordModel by request ID
    private PettyCashReplenishRecordModel findPettyCashRecordById(String requestId) {
        return pettyCashReplenishRequestData.stream()
            .filter(record -> record.getRequestId().equals(requestId))
            .findFirst()
            .orElse(null);
    }

    private void saveRequestApprovalWithBiometric(PettyCashReplenishRecordModel record, EmployeeSignatureModel approver, 
                                         byte[] fingerprintTemplate, String notes) {
        new Thread(() -> {
            try {
                boolean success = databaseConnector.savePettyCashReplenishRequestApprovalWithBiometric(
    ...
    private void saveDispenseApprovalWithBiometric(PettyCashReplenishRecordModel record, EmployeeSignatureModel approver, 
                                         byte[] fingerprintTemplate, String notes) {
        new Thread(() -> {
            try {
                boolean success = databaseConnector.savePettyCashReplenishDispenseApprovalWithBiometric(
                    record, approver, fingerprintTemplate, notes
                );
    ...
    private void saveConfirmationWithBiometric(PettyCashReplenishRecordModel record, EmployeeSignatureModel confirmer, 
                                             byte[] fingerprintTemplate, String notes) {
        new Thread(() -> {
            try {
                boolean success = databaseConnector.savePettyCashReplenishConfirmationWithBiometric(
    ...
    private void updateButtonStates(PettyCashReplenishRecordModel selected) {
        if (selected == null) {
    ...
    private void updateButtonTooltips(PettyCashReplenishRecordModel selected) {
...
private void viewRecordDetails(PettyCashReplenishRecordModel record) {
    if (record == null) {
...
//        logoView.setPreserveRatio(true);
        ...
        Label title = new Label("PETTY CASH FUND REPORT");
        ...
        Label requestIdLabel = new Label("PettyCash ID: " + getSafeString(record.getRequestId()));
        ...
        statusGrid.addRow(4,
         ...
         statusGrid.addRow(5,
            createBoldLabel("Void Reason:"), createStatusLabel(getSafeString(databaseConnector.getPettyCashReplenishRequestVoidReason(record.getRequestId()))));
        ...
        exportButton.setOnAction(e -> {
            ...
            exportNodeToPDF(contentWithButtons, "PettyCashRequest_" + record.getRequestId() + ".pdf");
        });
        ...
        if (proceed) {
            ...
            if (success) {
                ...
                showAlert("Print Successful", 
                    "Petty cash request details were sent to the printer.", 
                    Alert.AlertType.INFORMATION);
            } else {
    ...
    try {
        ...
        Platform.runLater(() -> {
            try {
                ...
                File documentsDir = new File(System.getProperty("user.home"), "Documents/PettyCashFunds/");
...
private void loadSignaturesForDetails(PettyCashReplenishRecordModel record, HBox signaturesContainer) {
...
//    private String getSignatureTitle(String owner, PettyCashReplenishRecordModel record) {
...
//    }
    ...
    private boolean hasAllSignatures(PettyCashReplenishRecordModel record) {
    try {
    ...
    private void printDateRangeTabular(LocalDate fromDate, LocalDate toDate) {
        List<PettyCashReplenishRecordModel> filteredData = pettyCashReplenishRequestData.stream()
    ...
    private void exportSelectedRowToExcelWithImages() {
        ...
        fileChooser.setInitialFileName("PettyCash_Details_" + selected.getRequestId() + "_" + LocalDate.now() + ".xlsx");
        ...
        if (file != null) {
            new Thread(() -> {
                try (Workbook workbook = new XSSFWorkbook()) {
                    ...
                    headerCell.setCellValue("AFRAN GENERAL HOSPITAL - PETTY CASH REQUEST DETAILS");
    ...
    private void exportDateRangeToExcel(LocalDate fromDate, LocalDate toDate) {
        ...
        fileChooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("Excel Files", "*.xlsx"));
        fileChooser.setInitialFileName("PettyCash_Report_" + fromDate + "_to_" + toDate + ".xlsx");
    ...
    private void exportDateRangeToExcelWithImages(LocalDate fromDate, LocalDate toDate) {
        ...
        fileChooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("Excel Files", "*.xlsx"));
        fileChooser.setInitialFileName("PettyCash_Detailed_Report_" + fromDate + "_to_" + toDate + ".xlsx");
    ...
    private void exportSelectedRowToWordWithImages() {
        ...
        fileChooser.setInitialFileName("PettyCash_Details_" + selected.getRequestId() + "_" + LocalDate.now() + ".docx");
        ...
        if (file != null) {
            new Thread(() -> {
                try (XWPFDocument document = new XWPFDocument()) {
                    ...
                    subtitleRun.setText("PETTY CASH REQUEST DETAILS");
    ...
    private void exportDateRangeToWord(LocalDate fromDate, LocalDate toDate) {
        ...
        fileChooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("Word Documents", "*.docx"));
        fileChooser.setInitialFileName("PettyCash_Report_" + fromDate + "_to_" + toDate + ".docx");
    ...
    private void exportDateRangeToWordWithImages(LocalDate fromDate, LocalDate toDate) {
        ...
        fileChooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("Word Documents", "*.docx"));
        fileChooser.setInitialFileName("PettyCash_Detailed_Report_" + fromDate + "_to_" + toDate + ".docx");
    ...
    private VBox createDetailedPrintNode(PettyCashReplenishRecordModel record, String title) {
        ...
        Label subHeader = new Label("PETTY CASH REQUEST MANAGEMENT SYSTEM");
    ...
    private VBox createTabularPrintNode(List<PettyCashReplenishRecordModel> data, LocalDate fromDate, LocalDate toDate) {
        VBox printNode = new VBox(15);
        ...
        Label subHeader = new Label("PETTY CASH REQUESTS SUMMARY REPORT");
        ...
        double totalAmount = data.stream().mapToDouble(PettyCashReplenishRecordModel::getAmountRequested).sum();
        ...
        // Table
        TableView<PettyCashReplenishRecordModel> printTable = new TableView<>();
        printTable.setItems(FXCollections.observableArrayList(data));
        
        TableColumn<PettyCashReplenishRecordModel, String> reqIdCol = new TableColumn<>("Request ID");
        reqIdCol.setCellValueFactory(new PropertyValueFactory<>("requestId"));
        
        TableColumn<PettyCashReplenishRecordModel, String> unitCol = new TableColumn<>("Unit");
        unitCol.setCellValueFactory(new PropertyValueFactory<>("requisitionUnit"));
        
        TableColumn<PettyCashReplenishRecordModel, String> payeeCol = new TableColumn<>("Payee");
        payeeCol.setCellValueFactory(new PropertyValueFactory<>("payee"));
        
        TableColumn<PettyCashReplenishRecordModel, Double> amountCol = new TableColumn<>("Amount");
        amountCol.setCellValueFactory(new PropertyValueFactory<>("amountRequested"));
        
        TableColumn<PettyCashReplenishRecordModel, String> statusCol = new TableColumn<>("Status");
    ...
    private VBox createDateRangeDetailedPrintNode(List<PettyCashReplenishRecordModel> data, LocalDate fromDate, LocalDate toDate) {
        VBox printNode = new VBox(15);
        ...
        Label subHeader = new Label("DETAILED PETTY CASH REQUESTS REPORT");
        subHeader.setStyle("-fx-font-size: 14; -fx-font-weight: bold; -fx-text-fill: #3498db; -fx-alignment: center;");
        ...
        for (int i = 0; i < Math.min(data.size(), 5); i++) { // Limit to 5 records for printing
            PettyCashReplenishRecordModel record = data.get(i);
    ...
    private VBox createRecordPrintBox(PettyCashReplenishRecordModel record, int index) {
    ...
    private void exportToExcelWithData(List<PettyCashReplenishRecordModel> data, File file, LocalDate fromDate, LocalDate toDate) {
        new Thread(() -> {
            try (Workbook workbook = new XSSFWorkbook()) {
                Sheet sheet = workbook.createSheet("Petty Cash Requests");
                ...
                titleCell.setCellValue("AFRAN GENERAL HOSPITAL - PETTY CASH REQUESTS REPORT");
                ...
                // Data rows
                int rowNum = 4;
                for (PettyCashReplenishRecordModel record : data) {
                    Row row = sheet.createRow(rowNum++);
                ...
                summaryCell.setCellValue("SUMMARY: Total Requests: " + data.size() + 
                    " | Total Amount: ETB " + data.stream().mapToDouble(PettyCashReplenishRecordModel::getAmountRequested).sum());
    ...
    private void exportToExcelWithImages(List<PettyCashReplenishRecordModel> data, File file, LocalDate fromDate, LocalDate toDate) {
    ...
    private void exportToWordWithData(List<PettyCashReplenishRecordModel> data, File file, LocalDate fromDate, LocalDate toDate) {
        new Thread(() -> {
            try (XWPFDocument document = new XWPFDocument()) {
                // Title
                XWPFParagraph titleParagraph = document.createParagraph();
                ...
                subtitleParagraph.setAlignment(ParagraphAlignment.CENTER);
                XWPFRun subtitleRun = subtitleParagraph.createRun();
                subtitleRun.setText("PETTY CASH REQUESTS REPORT");
                subtitleRun.setBold(true);
                subtitleRun.setFontSize(14);
                ...
                summaryRun.setText("Summary: Total Requests: " + data.size() + 
                    ...
                    data.stream().mapToDouble(PettyCashReplenishRecordModel::getAmountRequested).sum()));
    ...
    private void exportToWordWithImages(List<PettyCashReplenishRecordModel> data, File file, LocalDate fromDate, LocalDate toDate) {
    ...
    private VBox createRequestDetailsSection(PettyCashReplenishRecordModel record) {
...
filterBox.getChildren().addAll(
        ...
        pettyCashReplenishRequestTable = new TableView<>();
        pettyCashReplenishRequestTable.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY);
        pettyCashReplenishRequestTable.setStyle("-fx-background-color: #ecf0f1; -fx-border-color: #bdc3c7; -fx-border-radius: 5;");

        pettyCashReplenishRequestTable.setRowFactory(tv -> {
    TableRow<PettyCashReplenishRecordModel> row = new TableRow<PettyCashReplenishRecordModel>() {
        @Override
        protected void updateItem(PettyCashReplenishRecordModel item, boolean empty) {
    ...
    row.selectedProperty().addListener((obs, wasSelected, isNowSelected) -> {
        if (isNowSelected && row.getItem() != null) {
            ...
            PettyCashReplenishRecordModel item = row.getItem();
            if (item != null) {
                if ("Yes".equalsIgnoreCase(item.getVoidStatus())) {
    ...
    row.setOnMouseClicked(event -> {
        if (event.getClickCount() == 2 && !row.isEmpty()) {
            PettyCashReplenishRecordModel selectedRecord = row.getItem();
    ...
    return row;
        ...
        TableColumn<PettyCashReplenishRecordModel, String> requestIdCol = createStyledColumn("Request ID", "requestId", "#2c3e50");
        TableColumn<PettyCashReplenishRecordModel, String> requisitionUnitCol = createStyledColumn("Requisition Unit", "requisitionUnit", "#34495e");
        TableColumn<PettyCashReplenishRecordModel, String> reasonCol = createStyledColumn("Reason", "reason", "#16a085");
        TableColumn<PettyCashReplenishRecordModel, String> payeeCol = createStyledColumn("Payee", "payee", "#27ae60");
        TableColumn<PettyCashReplenishRecordModel, Double> amountRequestedCol = createStyledColumn("Amount Requested", "amountRequested", "#2980b9");
        TableColumn<PettyCashReplenishRecordModel, Double> amountAvailableCol = createStyledColumn("Amount Available", "availableAmount", "#2980b9");
        TableColumn<PettyCashReplenishRecordModel, String> currentStatusCol = createStyledColumn("Status", "currentStatus", "#2980b9");
        TableColumn<PettyCashReplenishRecordModel, LocalDate> requestDateCol = createStyledColumn("Request Date", "requestDate", "#8e44ad");
        TableColumn<PettyCashReplenishRecordModel, String> approvalStatusCol = createStyledColumn("Approval Status", "approvalStatus", "#c0392b");
        TableColumn<PettyCashReplenishRecordModel, String> approvedByCol = createStyledColumn("Approved By", "approvedBy", "#7f8c8d");
        TableColumn<PettyCashReplenishRecordModel, String> confirmationStatusCol = createStyledColumn("Confirmation Status", "confirmationStatus", "#f39c12");
        TableColumn<PettyCashReplenishRecordModel, String> confirmedByCol = createStyledColumn("Confirmed By", "confirmedBy", "#d35400");
        TableColumn<PettyCashReplenishRecordModel, String> dispensedStatusCol = createStyledColumn("Dispensed Status", "dispensedStatus", "#16a085");
        TableColumn<PettyCashReplenishRecordModel, String> dispensedByCol = createStyledColumn("Dispensed By", "dispensedBy", "#27ae60");
        TableColumn<PettyCashReplenishRecordModel, String> dispenseApprovalStatusCol = createStyledColumn("Dispense Approval Status", "dispenseApprovalStatus", "#d35400");
        TableColumn<PettyCashReplenishRecordModel, String> dispenseApprovedByCol = createStyledColumn("Dispense Approved By", "dispenseApprovedBy", "#8e44ad");
        TableColumn<PettyCashReplenishRecordModel, String> voidStatusCol = createStyledColumn("Void Status", "voidStatus", "#e74c3c");
        TableColumn<PettyCashReplenishRecordModel, String> voidedByCol = createStyledColumn("Voided By", "voidedBy", "#c0392b");

        pettyCashReplenishRequestTable.getColumns().addAll(
        ...
        pettyCashReplenishRequestTable.setItems(pettyCashReplenishRequestData);
        
        Label placeholder = new Label("No petty cash requests found. Click 'New Request' to create one.");
        placeholder.setStyle("-fx-text-fill: #7f8c8d; -fx-font-size: 14; -fx-padding: 20;");
        pettyCashReplenishRequestTable.setPlaceholder(placeholder);

        // Proper selection listener
        pettyCashReplenishRequestTable.getSelectionModel().selectedItemProperty().addListener((obs, oldSel, newSel) -> {
            if (newSel != null) {
        ...
        tableSection.getChildren().addAll(filterBox, pettyCashReplenishRequestTable);
        return tableSection;
    }
    
    
    private void exportSelectedRecordToPDF(PettyCashReplenishRecordModel record) {
    if (record == null) {
    ...
    try {
        ...
        // ===== EXPORT TO PDF =====
        String fileName = "PettyCashRequest_" + record.getRequestId() + ".pdf";
   ...
   private <T> TableColumn<PettyCashReplenishRecordModel, T> createStyledColumn(String title, String property, String color) {
    TableColumn<PettyCashReplenishRecordModel, T> column = new TableColumn<>(title);
    column.setCellValueFactory(new PropertyValueFactory<>(property));
    column.setStyle("-fx-background-color: " + color + "; -fx-text-fill: white; -fx-font-weight: bold;");
    
    column.setCellFactory(tc -> new TableCell<PettyCashReplenishRecordModel, T>() {
        @Override
    ...
    pettyCashReplenishRequestTable.setItems(
    pettyCashReplenishRequestData.filtered(record -> {
    ...
    private void refreshData() {
        ...
        showAlert("Refreshed", "Data refreshed successfully!\n" + 
                  "Loaded " + pettyCashReplenishRequestData.size() + " petty cash requests and " + 
    ...
    private void showAdvancedSearchDialog() {
        ...
        dialog.setHeaderText("Search Petty Cash Requests with Multiple Criteria");
        dialog.getDialogPane().setPrefSize(600, 500);
    ...
    private void performAdvancedSearch(String requestId, String payee, String requisitionUnit, 
        ...
        pettyCashReplenishRequestTable.setItems(pettyCashReplenishRequestData.filtered(record -> {
        ...
        showAlert("Search Complete", 
            "Found " + pettyCashReplenishRequestTable.getItems().size() + " matching records.",
            Alert.AlertType.INFORMATION);
    }
